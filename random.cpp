...
const double Random::AM = (1.0/ (double) Random::IM);

long int Random::set_seed_usec()
{
    struct timeval seed_usec;
    if (gettimeofday(&seed_usec, NULL) != 0)
    {
        printf("\nError in reading time of day");
        exit(-1);
    }
    else
    {
        printf("\nRNG seed set to %ld", seed_usec.tv_usec);
    }

    set_seed((long int) seed_usec.tv_usec);
    return seed_usec.tv_usec;
}
